    ...
    for i in range(1, N):
        ...
        for j in seq:
            ...
            for k in range(len(cond) - 1, -1, -1):
                ...
                # Clip values to avoid invalid sqrt
                
                Tk_clipped = np.clip(T[k], -1, 1)
                r1_clipped = np.clip(r1, -1, 1)
                
                sqrt_arg = (1 - Tk_clipped ** 2) * (1 - r1_clipped ** 2)
                sqrt_arg = max(sqrt_arg, 0)  # Avoid negative due to numerical errors

                shat = s * np.sqrt(sqrt_arg) + Tk_clipped * r1_clipped
                ...
                # shat = s * np.sqrt((1 - T[k] ** 2) * (1 - (r1) ** 2)) + T[k] * r1 # <-- Old code
                s = shat
            if np.isnan(s):
                R[SO[i], j] = s
                R[j, SO[i]] = s
...
def PartCorr(i, j, cond, R, L, m):
    ...
    # # Calculating partial correlation of [(i,j) | (cond(1),cond(2:n))] <-- OLD CODE
